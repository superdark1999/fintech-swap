...
// File: contracts/interfaces/ISwapNFT.sol
...
contract SwapNFTMarketPlace is ISwapNFT, ERC721Holder, Ownable, Pausable {
    using SafeMath for uint256;
    ...
    function getOfferByTokenIdAndAddressSwap(uint256 _tokenId, address _address)
     ...
     function offerNFT(uint256 _tokenId,uint256 _tokenTarget, uint256 _price) public override whenNotPaused {
        //  require(_listMap[_tokenTarget]!=0x0000000000000000000000000000000000000000, 'Token target not yet listing');
         if(_price>0){
             quoteErc20.safeTransferFrom(address(_msgSender()), address(this), _price);
         }
         nft.safeTransferFrom(address(_msgSender()), address(this), _tokenId);
        _offerMap[_tokenTarget].push(Offer({tokenId: _tokenId, price: _price, user:address(_msgSender())}));

...
interface IBidNFT {
    function buyToken(uint256 _tokenId, address _addressNFT) external;

    function buyTokenTo(uint256 _tokenId, address _to, address _addressNFT) external;
    ...
    function readyTokenOther(uint256 _tokenId, uint256 _price, address _addressNFT) external;
    
    function readyToBidToken(uint256 _tokenId, uint256 _price, uint256 _step,  address _addressNFT) external;

    function revokeBidToken(uint256 _tokenId, address _addressNFT) external;
    ...
    function cancelSellToken(uint256 _tokenId,  address _addressNFT) external;

    function bidToken(uint256 _tokenId, uint256 _price, address _addressNFT) external;

    function updateBidPrice(uint256 _tokenId, uint256 _price) external;

    function sellTokenTo(uint256 _tokenId, address _to,address _addressNFT) external;

    function cancelBidToken(uint256 _tokenId,address _addressNFT) external;
}
...
contract LuckyMarketPlace is IBidNFT, ERC721Holder, Ownable, Pausable {
    ...
    mapping(string => address) public _tokenSellers2;
    mapping(string => uint256) public _tokenSellersPrice2; 
    mapping(string => address) public _tokenSellersAddress;
    mapping(string => EnumerableSet.UintSet) private _userSellingTokens2;
    
    mapping(string => address) private _userOpenBid;    
    mapping(string => uint256) public _userOpenBidPrice;    
    mapping(string => address) private _tokenBidders2;
    mapping(string => BidEntry[]) private _tokenBids2;    
    ...
    event CancelSellToken(address indexed seller, uint256 indexed tokenId, address _addressNFT);
    ...
    event Bid(address indexed bidder, uint256 indexed tokenId, uint256 price, address _addressNFT);
    event CancelBidToken(address indexed bidder, uint256 indexed tokenId, address _addressNFT);
    ...
    function combineNFT(address _nft, uint256 _tokenId) public pure returns (string memory _uintAsString) {
          return string(abi.encodePacked(addressToString(_nft),uint2str(_tokenId)));
    }
    
    function readyTokenOther(
        uint256 _tokenId,
        uint256 _price,
        address _addressNFT
    ) public override whenNotPaused {
        require(_msgSender() == IERC721(_addressNFT).ownerOf(_tokenId), 'Only Token Owner can sell token');
        require(_price != 0, 'Price must be granter than zero');
        string memory key  =combineNFT(_addressNFT,_tokenId);
        
        IERC721(_addressNFT).safeTransferFrom(address(_msgSender()), address(this), _tokenId);
        
        _tokenSellers2[key] = msg.sender;
        _tokenSellersPrice2[key] = _price;
        
    }
    

    function buyToken(uint256 _tokenId,  address _addressNFT) public override whenNotPaused {
        buyTokenTo(_tokenId, _msgSender(),_addressNFT);
    }

    function buyTokenTo(uint256 _tokenId, address _to,  address _addressNFT) public override whenNotPaused {
        string memory key  =combineNFT(_addressNFT,_tokenId);
        uint256 price = _tokenSellersPrice2[key];
        require(price !=0, 'Token not in sell book');
        require(_msgSender() != address(0) && _msgSender() != address(this), 'Wrong msg sender');
        
        IERC721(_addressNFT).safeTransferFrom(address(this), _to, _tokenId);
        
        uint256 feeAmount = price.mul(feePercent).div(100);
        if (feeAmount != 0) {
            quoteErc20.safeTransferFrom(_msgSender(), feeAddr, feeAmount);
        }
        quoteErc20.safeTransferFrom(_msgSender(),_tokenSellers2[key], price.sub(feeAmount));
        
        
        // clear data seller
        _tokenSellersPrice2[key]=0;
        _tokenSellers2[key] =0x0000000000000000000000000000000000000000;
    }
    ...
    function cancelSellToken(uint256 _tokenId,  address _addressNFT) public override whenNotPaused {
        string memory key = combineNFT(_addressNFT,_tokenId);
        require(_tokenSellers2[key]==msg.sender, 'Only Seller can cancel sell token');
         IERC721(_addressNFT).safeTransferFrom(address(this), _msgSender(), _tokenId);
        // _asksMap.remove(_tokenId);
        _tokenSellersPrice2[key]=0;
        _tokenSellers2[key] =0x0000000000000000000000000000000000000000;
        emit CancelSellToken(_msgSender(), _tokenId,_addressNFT);
    }

    function readyToBidToken(uint256 _tokenId, uint256 _price, uint256 _step,  address _addressNFT) public override whenNotPaused {
        require(_msgSender() == IERC721(_addressNFT).ownerOf(_tokenId), 'Only Token Owner can sell token');
        require(_price != 0, 'Price must be granter than zero');
        ...
        _userOpenBid[key] = address(_msgSender());
        _userOpenBidPrice[key] = _price;
    }
    
    function revokeBidToken(uint256 _tokenId,  address _addressNFT) public override whenNotPaused {
        string memory key  =combineNFT(_addressNFT,_tokenId);        
        require(_userOpenBid[key]==msg.sender, 'Only Seller can cancel sell token');
        IERC721(_addressNFT).safeTransferFrom(address(this), _msgSender(), _tokenId);
       _userOpenBid[key]  = 0x0000000000000000000000000000000000000000;
       _userOpenBidPrice[key]  =0;
    ...
    function bidToken(uint256 _tokenId, uint256 _price, address _addressNFT) public override whenNotPaused {
        require(_msgSender() != address(0) && _msgSender() != address(this), 'Wrong msg sender');
        require(_price != 0, 'Price must be granter than zero');
        
        uint256 priceBid =_userOpenBidPrice[combineNFT(_addressNFT,_tokenId)];     
        
        require(priceBid!=0, 'Token not in bid book');
        require(_price>priceBid, 'Price must be granter than price bid');
    
        address _bidder = _tokenBidders2[combineNFT(_addressNFT,_tokenId)];
        address _to = address(_msgSender());
        require(_bidder!=_to, 'Bidder already exists');
        
        
        quoteErc20.safeTransferFrom(address(_msgSender()), address(this), _price);
        _tokenBids2[combineNFT(_addressNFT,_tokenId)].push(BidEntry({bidder: _to, price: _price}));
        emit Bid(_msgSender(), _tokenId, _price,_addressNFT);
    }
    ...
    function getBidByTokenIdAndAddress2(uint256 _tokenId, address _address, address _addressNFT)
        private
        view
        returns (BidEntry memory, uint256)
    {
        // find the index of the bid
        BidEntry[] memory bidEntries = _tokenBids2[combineNFT(_addressNFT,_tokenId)];
    ...
    function delBidByTokenIdAndIndex2(uint256 _tokenId, uint256 _index, address _addressNFT) private {
        _userBids[_tokenBids[_tokenId][_index].bidder].remove(_tokenId);
        // delete the bid
        uint256 len = _tokenBids2[combineNFT(_addressNFT,_tokenId)].length;
        for (uint256 i = _index; i < len - 1; i++) {
            _tokenBids2[combineNFT(_addressNFT,_tokenId)][i] = _tokenBids2[combineNFT(_addressNFT,_tokenId)][i + 1];
        }
        _tokenBids2[combineNFT(_addressNFT,_tokenId)].pop();
    }
    ...
    function sellTokenTo(uint256 _tokenId, address _to,address _addressNFT) public override whenNotPaused {
        string memory key  =combineNFT(_addressNFT,_tokenId);        
        require(_tokenSellersPrice2[key]!=0, 'Token not in sell book');
        address _seller = _tokenSellers2[key];
        address _owner = address(_msgSender());
        require(_seller == _owner, 'Only owner can sell token');
        // find  bid and the index
        (BidEntry memory bidEntry, uint256 _index) = getBidByTokenIdAndAddress2(_tokenId, _to,_addressNFT);
        require(bidEntry.price != 0, 'Bidder does not exist');
        ...
        delBidByTokenIdAndIndex2(_tokenId, _index, _addressNFT);
        emit Trade(_tokenSellers2[key], _to, _tokenId, price, feeAmount);
    }

    function cancelBidToken(uint256 _tokenId,address _addressNFT) public override whenNotPaused {
        address _address = address(_msgSender());
        require(_tokenBidders2[combineNFT(_addressNFT,_tokenId)]==_address, 'Only Bidder can cancel the bid');
        // find  bid and the index
        (BidEntry memory bidEntry, uint256 _index) = getBidByTokenIdAndAddress2(_tokenId, _address,_addressNFT);
        require(bidEntry.price != 0, 'Bidder does not exist');
        quoteErc20.transfer(_address, bidEntry.price);
        delBidByTokenIdAndIndex2(_tokenId, _index,_addressNFT);
        emit CancelBidToken(_msgSender(), _tokenId,_addressNFT);
    }

    function getBidsLength(uint256 _tokenId, address _addressNFT) public view returns (uint256) {
        return _tokenBids2[combineNFT(_addressNFT,_tokenId)].length;
    }

    function getBids(uint256 _tokenId, address _addressNFT) public view returns (BidEntry[] memory) {
        return _tokenBids2[combineNFT(_addressNFT,_tokenId)];
    }
